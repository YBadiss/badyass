...
def get_image(club_id: str) -> Image:
    try:
        response = requests.get(f'https://tmssl.akamaized.net//images/wappen/head/{club_id}.png')
...
def build_parent_club_map():
    # with open('./data/club_logo_hashes.json', 'r') as file:
    #     clubs_hash_map = json.load(file)
    with open('./data/clubs.json', 'r') as file:
        clubs = json.load(file)

    club_logos = {
        club['club_id']: get_image(club['club_id']) for club in tqdm(clubs, desc='Loading club logos')
    }
