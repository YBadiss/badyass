...
scp $SSH_OPTIONS root@167.71.143.97:/etc/nginx/sites-available/badyass nginx-config-${timestamp}
scp $SSH_OPTIONS nginx-config root@167.71.143.97:/etc/nginx/sites-available/badyass
...
if [ $? -ne 0 ]; then
    ...
    scp $SSH_OPTIONS nginx-config-${timestamp} root@167.71.143.97:/etc/nginx/sites-available/badyass
    exit 1
fi
